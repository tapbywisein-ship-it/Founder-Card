...
import { createEventSchema, updateEventSchema, searchEventsSchema, rsvpGuestSchema } from './events.validation';
...
router.get('/:id/attendees-public', authenticate, eventsController.listAttendeesForAttendee.bind(eventsController));
router.get('/:id/suggestions', authenticate, eventsController.getEventSuggestions.bind(eventsController));
...
router.get('/:id/checkin-token', authenticate, eventsController.getCheckInToken.bind(eventsController));
...
router.post('/:id/rsvp-guest', validate(rsvpGuestSchema), eventsController.rsvpGuest.bind(eventsController));

// Public — track a page view (anonymous or authed). Idempotent per session.
router.post('/:id/view', optionalAuthenticate, eventsController.trackPageView.bind(eventsController));

// Organizer — list visitors who viewed the event page.
router.get('/:id/visitors', authenticate, authorize('ORGANIZER', 'ADMIN'), eventsController.listVisitors.bind(eventsController));
